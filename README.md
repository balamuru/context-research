...
2. **Fan out** — `orchestrator.py` fetches from all four platforms in
   parallel-ish sequence via `fetchers.py`.
3. **Rank** — `ranker.py` puts engagement on a comparable log scale across
...
4. **Mine** — Claude reads the ranked data as a creative strategist and
   ...
   platforms.
5. **Brief** — `renderer.py` turns the mined brief into a shareable,
...
`orchestrator.py` writes ranked, normalized data only — it does no
content interpretation. The mined `brief.json` (hooks/pains/desires/etc.) is
Claude's output per `SKILL.md`'s schema, which `renderer.py` then turns into
the dashboard.
